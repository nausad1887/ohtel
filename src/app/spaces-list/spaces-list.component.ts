...
import { map, debounceTime, mergeMap, delay, distinctUntilChanged } from 'rxjs/operators';
...
export class SpacesListComponent implements OnInit, OnDestroy, DoCheck {
  public categorytypeID = '3';
  public subCategoryId = '46';
  public locationID = '';
  ...
  ngOnInit(): void {
    this.data.categoryID = this.activatedRoute.snapshot.paramMap.get('spaceID');
    this.updatedCity();
  ...
  public asyncFunctionCall = async () => {
    ...
    this.subCategoryTypeLists$ = this.homeService.spacesSubCategoryTypeList(this.data);
    this.subCATEGORYLists$ = this.homeService.spacesSubCategoryList(this.data);
    this.locationLists$ = this.homeService.getLocationLists(this.cityID);
    this.viewPostMethod();
  }
